...
std::tuple<std::vector<int>,std::vector<float>,std::vector<float>> GenVertexProducer::matchHadronsToSV(
    ...
    while (true) {
        ...
        if (common >= nRequiredCommonTracks) {
            ...
            distances[bestSV][bestHad] = 998.0;  // pair not matched
        }
    }
    ...
    // check the minDist not mathced using orignial matrix (not modified by matching procedure)
    ...
    return std::make_tuple(Hadron_SVIdx, Hadron_SVDistance, minDistNotMatched);
}
